...
var m_text = ["", "January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
...
function getEvents(e, data, td_id) {
	...
	if ($("#admin").val() == "true") {
		$("#addEvent").remove();
		$(".modal-header").append("<button class='btn btn-small btn-success add' id='addEvent' onclick=\"addEvent('"+ td_id + "');\">+ Add</button>");
	}
...
var modalAddEvent = { header: "<button type='button' class='close' data-dismiss='modal' aria-hidden='true'>&times;</button><h3 id='myModalLabel'>Add event</h3>",
						 body: "<form class='form-horizontal' id='addEvent' action=' method='post'>" +
									...
									"</div>" +
								"</form>",
						 footer: "<button type='button' id='add' class='btn btn-success' data-loading-text='Adding event...'>ADD EVENT</button>" };