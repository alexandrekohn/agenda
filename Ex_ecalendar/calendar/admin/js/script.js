var loading = "<td colspan='4' style='text-align: center; padding: 50px 0px;'>Loading events...</td>";
var not_found = "<td colspan='4' style='text-align: center; padding: 50px 0px;'>No events found</td>";
...
function action(action, data_id, data) {
	...
	this.edit = function (data_id, data) {
		...
		this.modal.footer.html("<button type='button' id='edelete' class='btn btn-primary' data-loading-text='Editing...' data-complete-text='The event has been edited!'>SAVE</button>");
	...
	this.del = function(data_id, data) {
		...
		this.modal.body.html("<h3 style='text-align: center'>You are going to delete:</h3><br /><h4 style='text-align: center'>" + data.replace(/&sQuote;/g, "\'").replace(/&dQuote;/g, "\"") + "</h4>");
			...
			complete: function() {
				if (action == "edit") {
					$(".header").after("<div class='alert alert-info'>" +
										"<a href='#' class='alert-link'>The event</a> has been edited correctly!" +
										"</div>");
				}
				else {
					$(".header").after("<div class='alert alert-danger'>" +
										"<a href='#' class='alert-link'>The event</a> has been deleted correctly!" +
										"</div>");
				}
	...
	$.ajax({
		...
		success: function(data) {
			...
			$(".header").after("<div class='alert alert-success'>" +
							   "<b>Well done!</b> <a href='#'' class='alert-link'>The event</a> has been created correctly!" +
							   "</div>");
		...
		error: function(err) { alert("There was an error trying to add the event."); }
	});
}
